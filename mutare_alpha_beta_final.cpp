...
const int Adancime_Alpha_Beta_Initial = 2;
const int Adancime_Alpha_Beta_Final = 3;
...
string comanda;
char culoare;
int nr_piese, nr_comenzi_jucate = 0, Adancime_Alpha_Beta;
...
int scoruri_piese[Nr_Jucatori + 1];
...
int evaluare_statica(int jucator){
    ...
    for(int i = 1; i <= Marime_Tabla; i++){
        for(int j = 1; j <= Marime_Tabla; j++){
            if(tabla_de_joc[i][j] == 0){
                ///Verific daca elementul curent este colt valid sau anulat pentru jucatorul curent
                e_colt_valid = 0;

                for(int dir = 0; dir < NrDirectii; dir++){
                    val = tabla_de_joc[i + dirColtX[dir]][j + dirColtY[dir]];
                    if(val != 0 && ((val % 2) == (jucator % 2))){
                        e_colt_valid = 1;
                    }
                }
                ...
                e_anulata = 0;
                for(int dir = 0; dir < NrDirectii; dir++){
                    val = tabla_de_joc[i + dirX[dir]][j + dirY[dir]];
                    if(val != 0 && ((val % 2) == (jucator % 2))){
                        e_colt_valid = 0;
                        e_anulata = 1;
                    }
                ...
                ///Verific daca elementul curent este colt valid sau anulat pentru adversarul jucatorului curent
                e_colt_valid = 0;
...
pair<mutare, int> calculare_mutari_posibile_si_alpha_beta(int nivel, int alpha, int beta, int jucator){
                    ...
                    if(tabla_de_joc[i][j] == 4){
                        ...
                        for(int h = 1; h <= piese[r].marime; h++){
                            ...
                            if(eValid){
                                ...
                                scoruri_piese[jucator] += piese[r].marime;
    ...
    if(ePass){
        alpha = evaluare_statica(jucator);
    }
...
void generare_mutare(){
    ...
    if(nr_comenzi_jucate <= 3){
        ...
        Adancime_Alpha_Beta = Adancime_Alpha_Beta_Final;
    }

    cin >> culoare;

    scoruri_piese[1] = 0; scoruri_piese[2] = 0;
